class Spree::Admin::BoardProductsController < Spree::Admin::ResourceController
  ...
  def index
    ...
    if params[:supplier_id]
      ...
      @board_products = @supplier.board_products.where( "isnull(spree_board_products.removed_at) and isnull(spree_board_products.approved_at)" ).includes({:product => [{:master => [:stock_items, :images]}]}, :board).page(params[:page] || 1).per(params[:per_page] || 5)
    else
      @board_products = Spree::BoardProduct.where( approved_at: nil, removed_at: nil).includes({:product => [{:master => [:stock_items, :images]}, :supplier]}, :board).page(params[:page] || 1).per(params[:per_page] || 5)
    end
